...
ui <- fluidPage(
    ...
    div(id = "display", style = "margin: 0 auto !important; max-width: 1000px; min-width: 1000px; float: none !important; padding: 10px; border-radius: 20px 10px 10px 10px; border: 1px solid #dddddd; position:relative", #
                    ...
                    tabPanel(value = "epidemic", title = iconTab("tab_epidemic", "Epidemic", "viruses"),
                             ...
                             fluidRow(
                                 column(4, 
                                        ...
                                        selectInput("epi_sim_time", label = "Simulate for", selected = 730, choices = c("1 year" = 365, "2 years" = 730, "3 years" = 1095, "4 years" = 1460, "5 years" = 1825)),
                                        sliderInput("epi_seed_size", label = "Starting number of infections", min = 1, max = 100, value = 10),
                                 ),
                                 column(4,
                                        sliderInput("epi_R0", "Basic reproduction number, R\u2080", min = 1.1, max = 4.5, value = 1.5, step = 0.1),
                                        sliderInput("epi_immune", "Proportion immune at start", min = 0, max = 1, value = 0, step = 0.01),
                                        sliderInput("epi_rho", "Case ascertainment rate", min = 0.01, max = 1, value = 1, step = 0.01)
                                 ),
                                 column(4,
                                        div(id = "epi_season_column",
                                            sliderInput("epi_season_phase", "Seasonality: peak week", min = 1, max = 52, value = 1, step = 1),
                                            sliderInput("epi_season_amp", "Seasonality: amplitude", min = 0.0, max = 1.0, value = 0, step = 0.01),
